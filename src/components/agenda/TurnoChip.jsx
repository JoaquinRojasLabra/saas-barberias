...
const styles = {
  ...
  "no-llego": "border-red-500/40 text-red-600",
...
export default function TurnoChip({ turno, cliente, servicio, onEstado }) {
  const Icon = turno.estado === "cumplido" ? CheckCircle : turno.estado === "no-llego" ? WarningCircle : turno.estado === "cancelado" ? XCircle : Clock
  ...
  return (
    <div className={cn("flex items-center justify-between gap-3 border rounded-xl px-4 py-3 surface", styles[turno.estado])}>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <button title="No llegó" onClick={() => onEstado(turno.id, "no-llego")} className="p-1.5 rounded-lg hover:bg-red-500/10 text-red-500"><WarningCircle size={16} /></button>
      </div>
    </div>
  )
}