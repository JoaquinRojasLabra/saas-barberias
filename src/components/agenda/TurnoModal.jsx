...
export default function TurnoModal({ onClose, empleadoPorDefecto }) {
  const { clientes, servicios, empleados, addTurno, esBarbero, session } = useStore()
  const push = useToast()
      ...
      <motion.div ref={ref} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="turno-modal-title" initial={{ scale: 0.95, y: 12 }} animate={{ scale: 1, y: 0 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-md max-h-[88vh] sm:max-h-[80vh] surface rounded-2xl p-5 sm:p-6 shadow-2xl outline-none overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-xs font-semibold text-[var(--fg-muted)]">Fecha</span>
              <input type="date" value={fecha} onChange={(e) => setFecha(e.target.value)} className="w-full mt-1 surface px-3 py-2 text-sm" />
            </label>
            <label className="block">
              <span className="text-xs font-semibold text-[var(--fg-muted)]">Hora</span>
              <input type="time" value={hora} onChange={(e) => setHora(e.target.value)} className="w-full mt-1 surface px-3 py-2 text-sm" />
            </label>
          </div>
          {!esBarbero && (
            <label className="block">
