...
export default function RegistroVenta({ onClose, onSave, empleadoPorDefecto }) {
  const { clientes, servicios, empleados, addVenta, generarLinkPago, esBarbero, session, negocio, datosPago } = useStore()
  const push = useToast()

  const metodos = []
  if (datosPago?.efectivo !== false) metodos.push("Efectivo")
  if (datosPago?.transferencia) metodos.push("Transferencia")
  if (datosPago?.mp && datosPago?.mpConfigurado) metodos.push("Mercado Pago")

  const [clienteId, setClienteId] = useState(clientes[0]?.id || "")
  const [servicioId, setServicioId] = useState(servicios[0]?.id || "")
  const [fechaHora, setFechaHora] = useState(`${hoyKey()}T10:30`)
  const [empleadoId, setEmpleadoId] = useState(empleadoPorDefecto || (esBarbero ? session?.barberoId : undefined) || empleados[0]?.id || "")
  ...
  const submit = async (e) => {
    ...
    setGuardando(true)
    try {
      const venta = await addVenta({ clienteId, servicioId, monto: servicio?.precio || 0, fechaHora, empleadoId, metodo })
      if (metodo === "Mercado Pago") {
        ...
        push("Pago en línea generado")
        window.open(link.init_point, "_blank", "noopener")
      }
      onSave?.()
      push("Venta registrada")
      onClose()
    } catch (err) {
      push(err?.message || "No se pudo registrar la venta", "error")
    } finally {
      setGuardando(false)
    }
  }
      ...
      <motion.div ref={ref} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="venta-modal-title" initial={{ scale: 0.95, y: 12 }} animate={{ scale: 1, y: 0 }} onClick={(e) => e.stopPropagation()} className="w-full max-w-md max-h-[88vh] sm:max-h-[80vh] surface rounded-2xl p-5 sm:p-6 shadow-2xl outline-none overflow-y-auto">
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          <label className="block">
            ...
            <select value={metodo} onChange={(e) => setMetodo(e.target.value)} disabled={metodos.length === 0} className="w-full mt-1 surface px-3 py-2 text-sm">
              {metodos.length === 0 ? <option value="Efectivo">Efectivo</option> : metodos.map((m) => <option key={m} value={m}>{m}</option>)}
            </select>
          </label>
          {metodo === "Mercado Pago" && (
            <p className="flex items-center gap-1.5 text-xs text-[var(--fg-muted)]">
              <LinkSimple size={14} className="text-[var(--accent)]" /> Se generará un pago de Mercado Pago para cobrar al cliente.
            </p>
          )}
          <p className="text-xs font-semibold text-[var(--fg-muted)]">Total: {formatCLP(servicio?.precio || 0)}</p>
          <button type="submit" disabled={!clienteId || !servicioId || !servicio?.precio || guardando} className="w-full mt-2 bg-[var(--accent)] text-white font-semibold py-2.5 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed">{guardando ? "Registrando…" : "Registrar venta"}</button>
        </form>
      </motion.div>
    </motion.div>
  )
